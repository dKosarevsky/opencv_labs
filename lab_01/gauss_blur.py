...
def description():
    ...
    4. Вывести на экран изображение в оттенках серого
    
    Задание на ЛР:
    Реализовать сглаживание изображения в оттенках серого с помощью фильтра Гаусса двумя способами:
     - с помощью встроенной функции GaussianBlur
     - вручную с помощью пространственной свёртки с ядром, пишем свою функцию свёртки (пригодится для других лаб)
     - вывести результат с наложенным фильтром
    """
...
def show_cols():
    c1, c2 = st.columns(2)
    with c1:
        dim = st.number_input("Размер ядра:", min_value=1, max_value=99, value=5, step=2)
    with c2:
        sig = st.number_input("Сигма (σ):", min_value=1, max_value=99, value=8, step=1)
